...
def getDnsRecords():
    with open('dns_records.txt', 'r') as f:
        ...
        for line in f:
            words = line.split()
            if words[2] != "Value" and words[2] != "A" and words[2] != "AAAA":
...
def handle_client(data, addr, server):
    while True:
        ...
        if str in dns_records and chkl :
            ...
            if addrs=="X" :
                ...
                packed_data = encode_msg(value+" MX")
                client.sendto(packed_data, TLD_ADDR)
                msg, addrr = client.recvfrom(SIZE)
                val,nxt = decode_msg(msg)
                
                #svaing in the cache
                cache[name] = (val,ttl,datetime.now())
