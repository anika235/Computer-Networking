...
def recv_msg(conn, addr):
    ...
    data = conn.recv(SIZE)
    if len(data) == 0 : # by deafult faka message send kore tai
        return
    ...
    if data["ORIGIN"]=="IGP": 
        ...
        for v in adj[ROUTER_NAME]:
            if (v=="AS2" or v=="AS3" or v=="AS4" or v=="AS1") and v != ROUTER_NAME:
                ...
                if v not in BGP_CONN:#bgp list e v ase naki
                    print(NAME_ADDR[v])
                    tmp = message(NAME_ADDR[v][0],NAME_ADDR[v][1])
                    BGP_CONN[v] = tmp
                
                if BGP_CONN[v].status=="OK":# v na thakle add korbe r thakle connection dekhbe
        ...
        if data["NEXT_HOP"] in adj[ROUTER_NAME]:# sem router e ase
            if data["HOST"] == data["NEXT_HOP"]:# host er kase back korse
