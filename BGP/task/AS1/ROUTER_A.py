...
def send_msg(inp):
    ...
    if inp in adj[ROUTER_NAME]:
        ...
        try:
            client.connect(("localhost",PORT_NAME[inp]))# nijer router moddhe connection
            print("-------------------------------")
            print(ROUTER_NAME)
            print(inp)
            print("-------------------------------")
    ...
    pickle_payload={"HOST" : ROUTER_NAME , # onno router e gele
                    "ORIGIN": "IGP" , 
                    "AS_PATH" : ROUTER_NAME , 
                    "NEXT_HOP" : inp , 
                    "METRIC" : adj[ROUTER_NAME][GATEWAY_NAME]}#local preference
...
def main():
    ...
    while True:
        ...
        if inp == "EXIT":
            break
        inp = inp.split()
        dst = inp[1]+"-"+inp[4]# host a of router as1 A-as1
